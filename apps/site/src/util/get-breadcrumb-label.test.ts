...
describe('getBreadcrumbLabel', () => {
  it('formats a simple two-level pathname', () => {
    expect(getBreadcrumbLabel('/article/article-name', 'https://example.com')).toBe(
  ...
  it('decodes URL segments and normalizes underscores', () => {
    expect(
      getBreadcrumbLabel('/help/contact_and_support%20team', 'https://example.com'),
    ).toBe('Help > Contact And Support Team');
  });

  it('removes file extensions from segments', () => {
    expect(getBreadcrumbLabel('/search/index.html', 'https://example.com')).toBe(
      'Search > Index',
    );
  });
